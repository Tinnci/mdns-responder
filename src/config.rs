...
impl ServiceConfig {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if !self.service_name.ends_with(".local.") {
            return Err(MdnsError::ConfigValidation("service_name must end with '.local.'".to_string()));
        }

        // Validate instance name is not empty
        if self.instance_name.is_empty() {
            return Err(MdnsError::ConfigValidation("instance_name cannot be empty".to_string()));
        }

        // Validate instance name length (max 63 characters per DNS label)
        if self.instance_name.len() > 63 {
            return Err(MdnsError::ConfigValidation("instance_name exceeds 63 characters".to_string()));
        }
        ...
        if self.hostname.is_empty() {
            return Err(MdnsError::ConfigValidation("hostname cannot be empty".to_string()));
        }

        // Validate hostname format (basic DNS compliance)
        if !self.hostname.chars().all(|c| c.is_alphanumeric() || c == '-') || self.hostname.starts_with('-') || self.hostname.ends_with('-') {
            return Err(MdnsError::ConfigValidation("hostname must contain only alphanumeric characters and hyphens".to_string()));
        }

        // Validate at least one share is configured
        if self.shares.is_empty() {
            return Err(MdnsError::ConfigValidation("at least one share must be configured".to_string()));
        }

        // Validate each share
        for (i, share) in self.shares.iter().enumerate() {
            if share.name.is_empty() {
                return Err(MdnsError::ConfigValidation(format!("share[{}]: name cannot be empty", i)));
            }
            if share.path.is_empty() {
                return Err(MdnsError::ConfigValidation(format!("share[{}]: path cannot be empty", i)));
            }
        }
